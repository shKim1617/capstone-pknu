...
if not cap.isOpened():
    ...
    while True:
        ...
        if right_fit is not None:  # 오른쪽 차선이 검출된 경우
            ...
            print(f"오른쪽 차선의 기울기 (slope): {slope_right}")
        else:
            print("오른쪽 차선이 검출되지 않았습니다.")

        video_centerx = mask_eroded.shape[0] 
        video_centery = mask_eroded.shape[1]

        # 오른쪽 차선 하단의 x 좌표 계산
        y_bottom = 360  # 이미지의 하단 y 좌표
        center_x = 640  # 이미지의 중앙 x 좌표

        if right_fit is not None:
            slope_right, intercept_right = right_fit  # 기울기와 절편
            x_right_bottom = slope_right * y_bottom + intercept_right  # 오른쪽 차선의 하단 x 좌표
            distance = abs(center_x - x_right_bottom)  # 거리 차이 계산
